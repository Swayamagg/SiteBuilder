...
import { Code2, MessageCircle, MessageSquare, Monitor, Rocket, Send, X } from "lucide-react";
...
const WebsiteEditor = () => {
  ...
  const thinkingSteps = ["Understanding your request...",
    ...
    <div className="h-screen w-screen flex bg-black text-white overflow-hidden">
      ...
      <div className=" flex-1 flex flex-col">
        <div className="h-14 px-4 flex items-center justify-between border-b border-black/10 bg-black/80">
          ...
          <div className="flex gap-2">
            {website.deployed ? "" :<button  onClick={handleDeployWebsite} className="flex items-center gap-2 px-4 py-1.5 rounded-lg bg-linear-to-r from-indigo-500 to-purple-500 text-sm font-semibold hover: scale-105 transition">
              <Rocket size={14} />
              Deploy
            </button>}
      ...
      <AnimatePresence>
        {showChat && (
          <motion.div initial={{y:"100%"}} animate={{y:"0%"}} exit={{y:"100%"}} className="fixed inset-0  z-[9999] bg-[#1e1e1e] flex flex-col">
      ...
      <AnimatePresence>
       {showCode &&(
        <motion.div initial={{x:"100%"}} animate={{x:"0%"}} exit={{x:"100%"}} className="fixed inset-y-0 right-0 w-full lg:w-[45%] z-[9999] bg-[#1e1e1e] flex flex-col">
          <div className="h-12 px-4 flex justify-between items-center border border-white/10 bg-[#1e1e1e]">
  ...
  function Header({onClose}) {
    return (
      <div className="h-14 px-4 flex items-center justify-between border-b border-white/10">
       ...
       {showChat &&(<button onClick={onClose}><X size={18} color="white"/></button>)} 
      </div>
    );
  }
};
